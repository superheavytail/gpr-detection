...
# @File     : train.py
#https://github.com/JamesYang568/Attention-U2Net
...
if __name__ == '__main__':
    ...
    from util.loss import multi_mix_loss_fusion as loss_fnc #이걸로!!
    #from util.loss import  bce_ssim_iou_loss as loss_fnc
    # from util.loss import multi_bce_loss_fusion as loss_fnc  #or 이걸로!!

    # ------- 2. set the directory of training dataset --------
    model_name = 'u3net'  # 'attu2net' 'u2net' 'AGCAOutsideRef' 'AGCAInsideRef'

    print(model_name)
    data_dir = os.path.join(os.getcwd(), 'train_data' + os.sep)
    tra_image_dir = os.path.join('train', 'imgs' + os.sep)
    tra_label_dir = os.path.join('train', 'gt' + os.sep)
    
    # ------- define validation dataset --------(내가 추가한거)
    val_image_dir = os.path.join('val', 'imgs' + os.sep)
    val_label_dir = os.path.join('val', 'gt' + os.sep)
    ...
    val_lbl_name_list = [data_dir + val_label_dir + os.path.splitext(os.path.basename(img_path))[0] + label_ext for img_path in val_img_name_list]
    ...
    save_frq = 100  # save the model every 100 iterations if run with cpu

    if torch.cuda.is_available():
        save_frq = 500  # save the model every 1500 iterations if run with gpu
        net.cuda()
    ...
    for epoch in range(0, epoch_num):
        ...
        with torch.no_grad():
            for i, val_data in enumerate(val_dataloader):
                ...
                val_loss2, val_loss = loss_fnc(val_outputs, val_labels)

                val_loss += val_loss.data.item()
                val_tar_loss += val_loss1.data.item()

            # del temporary outputs and loss
            del d0, d1, d2, d3, d4, d5, d6, loss2, loss


        # Calculate average validation loss (내가추가한거)
        average_val_loss = val_loss / val_num
        average_val_tar_loss = val_tar_loss / val_num

        print("Validation Loss: %3f, Validation TAR Loss: %3f" % (average_val_loss, average_val_tar_loss))
