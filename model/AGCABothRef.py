...
from model.AGCAInsideRef import AGCAU_Net7, AGCAU_Net6, AGCAU_Net5, AGCAU_Net4, AttU_Net4F
from model.u2net import _upsample_like


# Attention U2Net
class AGCABothRef(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        super(AGCABothRef, self).__init__()
...
# Attention U2Net Primer
class AGCABothRefP(nn.Module):

    def __init__(self, in_ch=3, out_ch=1):
        super(AGCABothRefP, self).__init__()
        ...
        self.attention3 = Attention_Gate(64, 64)
        self.stage3d = AGCAU_Net5(128, 16, 64)
        self.attention2 = Attention_Gate(64, 64)
    ...
    def forward(self, x):
        ...
        # stage 1  这个过程就是不断增加通道数，减少图片大小的过程
        hx1 = self.stage1(hx)  # 64*288*288
        hx = self.pool12(hx1)

        # stage 2
        hx2 = self.stage2(hx)  # 64*144*144
        hx = self.pool23(hx2)

        # stage 3
        hx3 = self.stage3(hx)  # 64*72*72
        hx = self.pool34(hx3)

        # stage 4
        hx4 = self.stage4(hx)  # 64*36*36
        hx = self.pool45(hx4)

        # stage 5
        hx5 = self.stage5(hx)  # 64*18*18
        hx = self.pool56(hx5)

        # stage 6
        hx6 = self.stage6(hx)  # 64*9*9

        # -------------------- decoder --------------------
        # ag5 = self.attention5(g=hx6, x=hx5)  #
        ca5 = self.cam5(hx6)  # 64*9*9
        hx6up = _upsample_like(ca5, hx5)  # 64*18*18
        hx5d = self.stage5d(torch.cat((hx6up, hx5), 1))  # 64*18*18

        # ag4 = self.attention4(g=hx5d, x=hx4)  #
        ca4 = self.cam4(hx5d)  # 64*18*18
        hx5dup = _upsample_like(ca4, hx4)  # 64*36*36
        hx4d = self.stage4d(torch.cat((hx4, hx5dup), 1))  # 64*36*36

        ag3 = self.attention3(g=hx4d, x=hx3)  # 64*36*36
        # ca3 = self.cam3(hx4d)  #
        hx4dup = _upsample_like(hx4d, hx3)  # 64*72*72
        hx3d = self.stage3d(torch.cat((ag3, hx4dup), 1))  # 64*72*72

        ag2 = self.attention2(g=hx3d, x=hx2)  # 64*72*72
        # ca2 = self.cam2(hx3d)  #
        hx3dup = _upsample_like(hx3d, hx2)  # 64*144*144
        hx2d = self.stage2d(torch.cat((ag2, hx3dup), 1))  # 64*144*144

        ag1 = self.attention1(g=hx2d, x=hx1)  # 64*144*144
        # ca1 = self.cam1(hx2d)  #
        hx2dup = _upsample_like(hx2d, hx1)  # 64*288*288
        hx1d = self.stage1d(torch.cat((ag1, hx2dup), 1))  # 64*288*288
