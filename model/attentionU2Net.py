...
from model.u2net import REBNCONV
...
class AttU_Net7(nn.Module):  # stage1  将图片从3维扩展到32维，但是尺寸从288变为9，再从9变为288（保持通过前后照片尺寸不变）
    ...
    def forward(self, x):
        ...
        hx6d = self.rebnconv6d(torch.cat((hx6, hx7), dim=1))  # 平行传递层 32, 9, 9

        # a5 = self.attention5(g=hx6d, x=hx5)  # 32, 18, 18
        hx6dup = _upsample_like(hx6d, hx5)  # 32, 18, 18
        hx5d = self.rebnconv5d(torch.cat((hx5, hx6dup), 1))  # 32, 18, 18
...
class AttU_Net6(nn.Module):  # 128,144,144  相比上一个stage通道扩大四倍，尺寸减小一倍
    ...
    def forward(self, x):  # 64,144,144  注意这里拿到的尺寸就已经降低了，这是因为外层pooling导致的
        ...
        hx5d = self.rebnconv5d(torch.cat((hx5, hx6), 1))  # 平行传递层
...
class AttU_Net5(nn.Module):  # 256,72,72
    ...
    def forward(self, x):  # 128,72,72
        ...
        hx4d = self.rebnconv4d(torch.cat((hx4, hx5), 1))  # 平行传递
...
class AttU_Net4(nn.Module):
    ...
    def forward(self, x):  # 256,36,36
        ...
        hx3d = self.rebnconv3d(torch.cat((hx3, hx4), 1))  # 平行传递层
        ...
        # a1 = self.attention1(g=hx2d, x=hx1)
        hx2dup = _upsample_like(hx2d, hx1)
        hx1d = self.rebnconv1d(torch.cat((hx1, hx2dup), 1))  # 512,36,36

        return hx1d + hxin


class AttU_Net4F(nn.Module):  # 不再使用升降采样改变尺寸和维度
    # 是一个退化类型——dilated convolution  注意这里也就不再进行注意力了，因为这里都是抽象语义，希望能够保留，注意力再次进行筛选的代价较高
    def __init__(self, in_ch=3, mid_ch=12, out_ch=3):
...
# AG only U2Net
class AttU2Net(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        super(AttU2Net, self).__init__()
...
# AG Only U2Net Primer
class AttU2NetP(nn.Module):

    def __init__(self, in_ch=3, out_ch=1):
        super(AttU2NetP, self).__init__()

        self.stage1 = AttU_Net7(in_ch, 16, 64)
        self.pool12 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage2 = AttU_Net6(64, 16, 64)
        self.pool23 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage3 = AttU_Net5(64, 16, 64)
        self.pool34 = nn.MaxPool2d(2, stride=2, ceil_mode=True)

        self.stage4 = AttU_Net4(64, 16, 64)
        self.pool45 = nn.MaxPool2d(2, stride=2, ceil_mode=True)
        ...
        self.stage4d = AttU_Net4(128, 16, 64)
        self.stage3d = AttU_Net5(128, 16, 64)
        self.stage2d = AttU_Net6(128, 16, 64)
        self.stage1d = AttU_Net7(128, 16, 64)
