# !/usr/bin/python
# -*- coding:utf-8 -*-
# @Time     : 2021/12/23 17:53
# @Author   : Yang Jiaxiong
# @File     : u2net.py
# Source    : U2Net from github
import torch
...
class U2NET(nn.Module):
    ...
    def forward(self, x):
        ...
        hx4d = self.stage4d(torch.cat((hx5dup, hx4), 1))  # 256*36*36
        hx4dup = _upsample_like(hx4d, hx3)  # 256*72*72

        hx3d = self.stage3d(torch.cat((hx4dup, hx3), 1))  # 128*72*72
        hx3dup = _upsample_like(hx3d, hx2)  # 128*144*144

        hx2d = self.stage2d(torch.cat((hx3dup, hx2), 1))  # 64*144*144
        hx2dup = _upsample_like(hx2d, hx1)  # 64*288*288

        hx1d = self.stage1d(torch.cat((hx2dup, hx1), 1))  # 64*288*288
